...
for i = 1:execs
    ...
    // Camada Oculta
    Z_train = zeros(n_train, n_neurons);
    C = rand(n_neurons, n_feat, 'normal');
    
    for n = 1:n_train
        for m = 1:n_neurons
            norma = norm(train_x(n,:)-C(m,:));
            Z_train(n, m) = exp(-norma^2)
        end
    end   
    
    Zt_train = Z_train';
    Z_train = [(-1)*ones(1, n_train); Zt_train]
    M = train_y'*Z_train'*(Z_train*Z_train')^(-1);
   
    // Camada de Saída
    Z_test = zeros(n_test, n_neurons);
    
    for n = 1:n_test
        for m = 1:n_neurons
            norma = norm(test_x(n,:)-C(m,:));
            Z_test(n, m) = exp(-norma^2)
        end
    end
    
    Zt_test = Z_test';
    Z_test = [(-1)*ones(1, n_test); Zt_test]
    output = M*Z_test
